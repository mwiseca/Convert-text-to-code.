...
void choice() {
	...
	cout << "//Enter cg for getline with error checking.\n";
...
void mapArrays() {
    ...
    std::string ifStatement [11] = {"    if(strcmp(",", ","[","]",") == 0) {"," = ",";","}","    }","}","        "};
    ...
    while(true) {
        ...
        std::cout << ifStatement[0] << select << ifStatement[1] << keyArray << ifStatement[2] << forLoopInt << ifStatement[3] << ifStatement[4] << "\n";
